...
$scriptsTarget = Join-Path ([IO.Path]::GetTempPath()) ("claude-agy-scripts-" + [guid]::NewGuid().ToString("N"))
try {
    ...
    & (Join-Path $root "scripts\install-for-antigravity.ps1") `
        -DestinationRoot $target -ScriptsRoot $scriptsTarget | Out-Null
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $scriptsTarget "claude-wrapper.ps1"))) { throw "scripts missing" }
    if (Test-Path -LiteralPath (Join-Path $target "scripts\claude-wrapper.ps1")) { throw "scripts placed under destination root" }
    Write-Output "PASS: Antigravity CLI installer"
} finally {
    if (Test-Path -LiteralPath $target) { Remove-Item -LiteralPath $target -Recurse -Force }
    if (Test-Path -LiteralPath $scriptsTarget) { Remove-Item -LiteralPath $scriptsTarget -Recurse -Force }
}
