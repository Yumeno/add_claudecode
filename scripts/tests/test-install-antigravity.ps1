...
try {
    ...
    & (Join-Path $root "scripts\install-for-antigravity.ps1") -DestinationRoot $target | Out-Null
    foreach ($source in Get-ChildItem -LiteralPath (Join-Path $root ".agents\skills") -Directory) {
        ...
        if (-not (Test-Path -LiteralPath (Join-Path $target ("skills\" + $source.Name + "\scripts")))) { throw "missing scripts for $($source.Name)" }
    }
