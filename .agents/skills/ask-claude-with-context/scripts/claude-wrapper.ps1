...
function Get-ConfigFilePath {
    ...
    $globalConfig = Join-Path $userProfile ".agents\add_claudecode\claude-wrapper.conf"
    $agentsMarker = [IO.Path]::DirectorySeparatorChar + ".agents" + [IO.Path]::DirectorySeparatorChar + "skills" + [IO.Path]::DirectorySeparatorChar
    $geminiSkillsMarker = [IO.Path]::DirectorySeparatorChar + ".gemini" + [IO.Path]::DirectorySeparatorChar + "skills" + [IO.Path]::DirectorySeparatorChar
    $antigravitySkillsMarker = [IO.Path]::DirectorySeparatorChar + ".gemini" + [IO.Path]::DirectorySeparatorChar + "antigravity-cli" + [IO.Path]::DirectorySeparatorChar + "skills" + [IO.Path]::DirectorySeparatorChar

    $agentsIndex = $scriptDir.IndexOf($agentsMarker, [StringComparison]::OrdinalIgnoreCase)
    if ($agentsIndex -ge 0) {
        $prefix = $scriptDir.Substring(0, $agentsIndex)
        if ($userProfile -and $prefix.TrimEnd('\') -ieq $userProfile.TrimEnd('\')) {
            return $globalConfig
        }
        return (Join-Path $prefix "scripts\claude-wrapper.conf")
    }

    if ($scriptDir.IndexOf($geminiSkillsMarker, [StringComparison]::OrdinalIgnoreCase) -ge 0 -or
        $scriptDir.IndexOf($antigravitySkillsMarker, [StringComparison]::OrdinalIgnoreCase) -ge 0) {
        return $globalConfig
    }

    return (Join-Path $scriptDir "claude-wrapper.conf")
}
