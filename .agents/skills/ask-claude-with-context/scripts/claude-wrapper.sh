...
config_file_for_script_dir() {
  case "$SCRIPT_DIR/" in
    */.agents/skills/*)
      local prefix=${SCRIPT_DIR%%/.agents/skills/*}
      if [[ -n "${HOME:-}" && "$prefix" == "$HOME" ]]; then
        printf '%s\n' "$HOME/.agents/add_claudecode/claude-wrapper.conf"
      else
        printf '%s\n' "$prefix/scripts/claude-wrapper.conf"
      fi
      ;;
    */.gemini/skills/*|*/.gemini/antigravity-cli/skills/*)
      printf '%s\n' "${HOME:?HOME is not set}/.agents/add_claudecode/claude-wrapper.conf"
      ;;
    *)
      printf '%s\n' "$SCRIPT_DIR/claude-wrapper.conf"
      ;;
  esac
}
CONFIG_FILE=$(config_file_for_script_dir)
...
if [[ -n "$SET_MODEL" ]]; then
  ...
  mkdir -p -- "$(dirname -- "$CONFIG_FILE")" || fail 1 "cannot create config directory: $(dirname -- "$CONFIG_FILE")"
