...
`SKILL.md` ベースの Agent Skills 形式（[agentskills.io](https://agentskills.io) 標準）で実装。Codex CLI の公式manualではユーザーSkillを `$HOME/.agents/skills/`、リポジトリ内Skillを `.agents/skills/` から読む仕様です。Gemini CLI も Agent Skills および `.agents/skills` alias 対応版で利用可能です（**1 セットのスキルで両方のツールから利用できる**ことを意図しています。Gemini 側の `.agents` alias 対応はバージョン依存があるため、認識されない場合は `~/.gemini/skills/` にも配置してください）。
...
foreach ($base in @("$env:USERPROFILE\.agents", "$env:USERPROFILE\.gemini")) {
    New-Item -ItemType Directory -Force "$base\skills" | Out-Null
...
`~/.agents/skills/` は Codex CLI 公式manual上のユーザーSkillディレクトリです。`~/.codex/` は `AGENTS.md`、`config.toml`、認証、session、plugin/cacheなどCodex home側の領域であり、Skillのcanonicalな配置先ではありません。Gemini CLI は `~/.gemini/skills/` を一次扱いし、`~/.agents/skills/` は alias として扱う実装になっているため、両方に置くのが確実です。

各Skillは、標準で必要なhelperを `$SKILL_DIR/scripts/` に同梱しています。`~/.agents/scripts/` や `~/.gemini/scripts/` へhelperを別途コピーする必要はありません。
...
Codex CLI のみ使う、または Gemini CLI が `.agents` alias を認識する版なら `~/.agents/skills/` だけでも動きます。Codexで認識されない場合は、まず `/skills` と起動ログで discovery path を確認してください。公式manual上のuser Skill位置は `$HOME/.agents/skills` です。
...
Codexと共通です。専用installerはhelper同梱済みのglobal SkillをこのAntigravity CLI専用領域へコピーします。
...
| Skill同梱helper | `~/.gemini/antigravity-cli/skills/<skill>/scripts/` |
...
モデル設定を消す場合は `~/.agents/add_claudecode/claude-wrapper.conf` も削除します。
...
| bash | PowerShell | 説明 |
|---|---|---|
| `--prompt` | `-Prompt` | （`--prompt-file` と排他、いずれか必須）Claude に送る質問 |
| `--prompt-file` | `-PromptFile` | 質問本文をUTF-8で書いたファイルから読む(非ASCII質問の回避策) |
...
`claude-wrapper.conf`、Claude Code既定の順です。未指定時のモデル名は推測表示しません。
Skill同梱wrapperから実行した場合でも、設定ファイルはSkillごとに分散させず、
配置元に応じた共通位置を使います。

| 配置 | 設定ファイル |
|---|---|
| repo内 `.agents/skills/.../scripts/` | repo直下 `scripts/claude-wrapper.conf` |
| user/global Skill | `~/.agents/add_claudecode/claude-wrapper.conf` |
...
`.git/config` / `.git/hooks/*`) の編集が必要な場合、`-Allow` / `--allow` に
exact pathを渡してユーザー承認済みの変更として通します。PowerShellの多値指定は
`powershell -File`のCLI境界仕様のためcomma区切り1引数: `-Allow ".env.example,.env.sample"`。
bashは `--allow` を繰り返し指定できます。

以下の条件では snapshot が fail closed で拒否されます: (a) リポジトリが submodule を
含む(`.gitmodules` または `.git/modules/*` の存在)、(b) リポジトリ内にリポジトリ外を
指す symlink / reparse point が存在する。

## Windows で非ASCII path (日本語など) を扱う

`powershell -File claude-implement.ps1 -Repo <path>` を bash などの UTF-8 shell 経由で
呼ぶ場合、CLI argv 境界で path が CP932 として mangling される可能性があります。回避策として
`-RepoFile` / `--repo-file` にUTF-8で path を書いたASCII名の一時ファイルを渡します。
...
同様に `claude-wrapper` は `-Prompt` の代わりに `-PromptFile` / `--prompt-file` で
非ASCII質問文をUTF-8ファイル経由で渡せます。`-Repo` / `-Prompt` と `-RepoFile` / `-PromptFile`
は排他で、どちらか一方が必須です。ASCII path / prompt なら従来通り `-Repo` / `-Prompt` を使えます。
...
powershell -ExecutionPolicy Bypass -NoProfile -File scripts\tests\test-skill-bundles.ps1
```
...
- Codex: 公式manual上のuser Skill位置は `~/.agents/skills/`。`~/.codex/` は `AGENTS.md`、設定、認証、session、plugin/cacheなどCodex home側の領域。
