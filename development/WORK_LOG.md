...
## 2026-07-07: Issue #4 Codex Skill配置とhelper同梱の整理

### 背景

別端末でのインストール検証から、README / SKILL.md の説明と実際のhelper探索が分かりにくい
というIssue #4が作成された。Codex manualを確認し、Codexのcanonicalなuser Skill位置は
`$HOME/.agents/skills`、`~/.codex`はAGENTS.md、config、auth、session、plugin/cacheなどの
Codex homeであると整理した。

### 対応方針

- `~/.codex/skills`はcanonicalな配置先として案内しない。
- 各Skillは、必要なhelperを `$SKILL_DIR/scripts/` に同梱する自己完結型へ寄せる。
- `~/.agents/scripts` / `~/.gemini/scripts` へhelperを置く方式は廃止する。
- `set-claude-model` の設定がSkillごとに分散しないよう、同梱wrapperでもbundle単位の
  共通 `claude-wrapper.conf` を使う。

### 実装

- `ask-claude`、`ask-claude-with-context`、`set-claude-model`、`list-claude-models`へ
  `claude-wrapper.ps1` / `.sh` を同梱した。
- `claude-implement`へ `claude-implement.ps1` / `.sh` と `claude-verify.ps1` / `.sh` を同梱した。
- `scripts/claude-wrapper.*` に共通config解決を追加した。
  - repo内Skill: repo直下 `scripts/claude-wrapper.conf`
  - user/global Skill: `~/.agents/add_claudecode/claude-wrapper.conf`
- READMEと各 `SKILL.md` を `$SKILL_DIR/scripts/` に一本化する説明へ更新した。
- `scripts/tests/test-skill-bundles.ps1` / `.sh` を追加し、同梱helperの存在とcanonical helperとの
  内容一致を検証するようにした。

### 検証

- `test-wrapper.ps1` / `.sh`
- `test-skill-bundles.ps1` / `.sh`
- `test-install-antigravity.ps1` / `.sh`
- `test-context.ps1` / `.sh`
- `test-verify.ps1` / `.sh`
- `test-implement.ps1` / `.sh`
- `git diff --check`

symlink系テストはWindows権限都合で既存通りskipされた。
